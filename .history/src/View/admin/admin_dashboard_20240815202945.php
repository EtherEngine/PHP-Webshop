...
// Datenbankabfragen für Bestellungen, Produkte und Benutzeraktivitäten
$sqlOrders = "
    SELECT 
...
$sqlRevenueTrend = "SELECT DATE(orders.order_date) AS date, SUM(order_items.price * order_items.quantity) AS total FROM orders JOIN order_items ON orders.id = order_items.order_id GROUP BY DATE(orders.order_date)";
...
$sqlOrderStatus = "SELECT orders.status, COUNT(*) AS total FROM orders GROUP BY orders.status";
...
$sqlOrderHeatmap = "SELECT HOUR(orders.order_date) AS hour, COUNT(*) AS total FROM orders GROUP BY HOUR(orders.order_date)";
$stmt = $pdo->prepare($sqlOrderHeatmap);
$stmt->execute();
$orderHeatmapData = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Daten für Verkäufe nach Land
$sqlSalesByCountry = "SELECT users.country, SUM(order_items.price * order_items.quantity) AS total_sales FROM users JOIN orders ON users.id = orders.user_id JOIN order_items ON orders.id = order_items.order_id GROUP BY users.country";
$stmt = $pdo->prepare($sqlSalesByCountry);
$stmt->execute();
$salesByCountryData = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Geokoordinaten für Länder (Beispielkoordinaten, dies müsste in einer echten Implementierung erweitert werden)
$countryCoordinates = [
...
<body>
    <div class="container mt-5">
        ...
        <!-- Interaktive Timeline für Benutzeraktivitäten -->
    ...
    <script>
        ...
        // Verkäufe nach Land (Leaflet Karte)
        ...
        <?php foreach ($salesByCountryData as $country => $data): ?>
            var coords = <?php echo json_encode($countryCoordinates[$country]); ?>;
            if (coords) {
                var marker = L.marker([coords.lat, coords.lng]).addTo(map);
                marker.bindPopup("<b><?php echo htmlspecialchars($country); ?></b><br>Umsatz: <?php echo number_format($data['total_sales'], 2, ',', '.'); ?> €");
            }
        <?php endforeach; ?>
