...
// Daten für Verkäufe nach city
$sqlSalesBycity = "SELECT city, SUM(price * quantity) AS total_sales FROM users JOIN orders ON users.id = orders.user_id JOIN order_items ON orders.id = order_items.order_id GROUP BY city";
$stmt = $pdo->prepare($sqlSalesBycity);
$stmt->execute();
$salesBycityData = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<body>
    ...
    <script>
        ...
        // Verkäufe nach city (Leaflet Karte)
        ...
        <?php foreach ($salesBycityData as $city): ?>
            var marker = L.marker([<?php echo $city['latitude']; ?>, <?php echo $city['longitude']; ?>]).addTo(map);
            marker.bindPopup("<b><?php echo htmlspecialchars($city['city']); ?></b><br>Umsatz: <?php echo number_format($city['total_sales'], 2, ',', '.'); ?> €");
        <?php endforeach; ?>
