...
<script>
    ...
    function destroyParticles() {
        ...
        if (particlesContainer) {
            particlesContainer.innerHTML = ''; // Löscht die Partikel-Canvas
            particlesJS && particlesJS.destroy(); // Zerstört die Partikel-Instanz, falls vorhanden
        }
    }

    function handleResize() {
        if (window.innerWidth <= 768) {
            destroyParticles(); // Deaktiviert Partikel bei kleiner Bildschirmbreite
        } else {
            initParticles(); // Aktiviert Partikel bei größerer Bildschirmbreite
        }
    }

    window.addEventListener('resize', handleResize);
    window.addEventListener('DOMContentLoaded', function() {
        handleResize(); // Initiale Prüfung bei Seitenladen
    });
</script>
