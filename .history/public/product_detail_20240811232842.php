    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add-to-cart').on('click', function () {
                clearTimeout(buttonTimeout);  // Clear any existing timeout
                clickCount++;
                var button = $(this);
                ...
                if (clickCount === 1) {
                    ...
                    // Double click or click on the "Add Quantity" button
                    var productId = button.data('id');
                    var quantity = popup.find('.quantity-input').val();

                    $.ajax({
                        type: 'POST',
                        url: 'product_detail.php?id=' + productId,
                        ...
                        success: function (response) {
                            ...
                            if (data.status === 'success') {
                                ...
                                $('.alert').remove();  // Remove any existing alert
                                $('body').append('<div class="alert alert-success">Produkt wurde zum Warenkorb hinzugefügt!</div>');
                                setTimeout(function () {
                                ...
                                popup.fadeOut('slow');
                                clickCount = 0;  // Reset click count after adding to cart
                            }
                        }
                    });
                }
