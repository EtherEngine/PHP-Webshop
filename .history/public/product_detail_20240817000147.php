...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Ähnliche Produkte basierend auf tags, name oder preis abrufen
    $tags = explode(',', $product['tags']);
    $tagsLike = '%' . implode('%', $tags) . '%';
    $priceRange = [$product['price'] - 50, $product['price'] + 50]; // Preisbereich von +/- 50€
    $similarStmt = $pdo->prepare(
        'SELECT * FROM products WHERE (tags LIKE ? OR name LIKE ? OR (price BETWEEN ? AND ?)) AND id != ? LIMIT 8'
    );
                                ...
                                <a href="product_detail.php?id=<?php echo $similarProduct['id']; ?>" class="card"
                                    ...
                                    <div class="card-body">
                                        ...
                                        <p class="card-text rating">
                                            <?php
                                            // Beispiel für Sternebewertung (fester Wert, später dynamisch machen)
                                            $rating = 4; // Zum Beispiel
                                            for ($i = 0; $i < 5; $i++) {
                                                if ($i < $rating) {
                                                    echo '★';
                                                } else {
