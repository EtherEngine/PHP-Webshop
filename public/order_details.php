...
$vat_amount = $total * $vat_rate; // Mehrwertsteuer auf den Warenkorb
$vat_on_shipping = $shipping_cost * $vat_rate; // Mehrwertsteuer auf Versandkosten (falls zutreffend)
$total_vat = $vat_amount + $vat_on_shipping; // Gesamte Mehrwertsteuer

// Gesamtsumme inklusive Versand und Mehrwertsteuer
$total_with_shipping_and_vat = $total + $shipping_cost + $total_vat;
?>
...
<body>
        ...
        <?php if (!empty($cart)): ?>
            <table class="table">
                ...
                <tfoot>
                    ...
                    <tr>
                        <td colspan="3" class="text-right"><strong>Versandkosten (Netto):</strong></td>
                        <td id="shipping_cost"><?php echo number_format($shipping_cost, 2, ',', '.'); ?> €</td>
                    </tr>
                    <tr>
                        <td colspan="3" class="text-right"><strong>Mehrwertsteuer
                                (<?php echo ($vat_rate * 100); ?>%):</strong></td>
                        <td id="vat_amount"><?php echo number_format($total_vat, 2, ',', '.'); ?> €</td>
                    </tr>
                    <tr>
                        <td colspan="3" class="text-right"><strong>Gesamtsumme inkl. Versand und MwSt.:</strong></td>
                        <td id="total_with_shipping_and_vat">
                            <?php echo number_format($total_with_shipping_and_vat, 2, ',', '.'); ?> €</td>
                    </tr>
                </tfoot>
            </table>
    ...
    <script>
        $(document).ready(function () {
            // Überwache Änderungen der Versandmethode
            $('#shipping_method').change(function () {
                // Holen der aktuellen Werte
                var shippingMethod = $(this).val();
                var totalWeight = <?php echo $total_weight; ?>;
                var country = '<?php echo $user_data['country']; ?>';
                var total = <?php echo $total; ?>;

                // AJAX-Request zur Berechnung der Versandkosten und Mehrwertsteuer
                $.ajax({
                    ...
                    success: function (response) {
                        // Aktualisieren der Versandkosten und Gesamtsumme
                        var shippingCost = parseFloat(response.shipping_cost);
                        var vatRate = <?php echo $vat_rate; ?>;
                        var vatAmount = (total + shippingCost) * vatRate;
                        var totalWithShippingAndVat = total + shippingCost + vatAmount;

                        // Anzeigen der aktualisierten Werte
                        $('#shipping_cost').text(shippingCost.toFixed(2).replace('.', ',') + ' €');
                        $('#vat_amount').text(vatAmount.toFixed(2).replace('.', ',') + ' €');
                        $('#total_with_shipping_and_vat').text(totalWithShippingAndVat.toFixed(2).replace('.', ',') + ' €');
                    },
